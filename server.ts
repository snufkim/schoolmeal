...
async function startServer() {
  ...
  // It uses process.env.GEMINI_API_KEY. It must have 'User-Agent': 'aistudio-build'
  ...
  // 3. Gemini Lunch Review
  app.post("/api/lunch-review", async (req, res) => {
    const { schoolName, mealName, dishes, concept } = req.body;
    if (!dishes || !Array.isArray(dishes) || dishes.length === 0) {
      return res.status(400).json({ error: "Dishes array is required" });
    }
    if (!process.env.GEMINI_API_KEY) {
      return res.status(400).json({ error: "Gemini API key is not configured in the environment. Please add it in Settings > Secrets." });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: "Failed to generate review. Please check Gemini API key configuration in Settings > Secrets." });
    }
  });
